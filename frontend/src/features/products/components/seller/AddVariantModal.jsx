...
export default function AddVariantModal({
  ...
  const handleImageChange = (e) => {
    ...
    if (files.length + variantImages.length > 5) {
  ...
  const handleSubmit = (e) => {
    ...
    if (!Object.keys(attributesObj).length) {
      toast.error("Provide at least one attribute (Size or Color)");
      return;
    }
    const finalPrice =
      priceAmount !== "" ? Number(priceAmount) : Number(basePrice);
    ...
    onSubmit(fd, () => {
      ...
      setCustomAttributes([]);
      setStock(20);
      setPriceAmount(basePrice || "");
      setPriceCurrency(baseCurrency || "INR");
      setVariantImages([]);
      setImagePreviews([]);
    });
  };
  ...
  const inputCls =
    "w-full border border-slate-200 rounded-xl bg-white px-3.5 py-2.5 text-sm text-slate-900 placeholder:text-slate-400 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/10 transition-all";
    ...
    >
      <div className='bg-white w-full max-w-lg rounded-2xl shadow-2xl border border-slate-200/50 flex flex-col max-h-[90vh]'>
        ...
        <div className='flex items-center justify-between px-6 py-5 border-b border-slate-100'>
          <div>
            ...
            <p className='text-xs text-slate-500 mt-0.5'>
              Specify attributes, stock, and optional price override.
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className='px-6 py-5 space-y-6 overflow-y-auto flex-1'>
          ...
          <div className='space-y-4'>
            ...
            {customAttributes.map((attr, idx) => (
              <div key={idx} className='flex items-center gap-2'>
                ...
                <button
                  type='button'
                  onClick={() =>
                    setCustomAttributes((p) => p.filter((_, i) => i !== idx))
                  }
            ...
            <button
              type='button'
              onClick={() =>
                setCustomAttributes((p) => [...p, { key: "", value: "" }])
              }
              ...
              <div>
                ...
                <div className='flex gap-1.5'>
                  <select
                    ...
                    className='text-xs font-bold bg-slate-100 border border-slate-200 rounded-xl px-2 py-2.5 outline-none cursor-pointer'
                  >
          ...
          <div className='space-y-3 pt-2 border-t border-slate-100'>
              ...
              {imagePreviews.map((url, idx) => (
                ...
                >
                  ...
                  <button
                    ...
                    className='absolute top-0.5 right-0.5 w-4 h-4 rounded-full bg-black/65 text-white flex items-center justify-center text-[9px] cursor-pointer'
                  >
                    ✕
                  </button>
                </div>
            ...
            <p className='text-[11px] text-slate-400'>
              If no images are uploaded, the product's default image will be
              used.
            </p>
          </div>
        </div>
        ...
        <div className='px-6 py-4 border-t border-slate-100 bg-slate-50 rounded-b-2xl flex items-center justify-end gap-3'>
          <button
            onClick={onClose}
            className='px-5 py-2.5 rounded-full text-xs font-semibold text-slate-700 bg-white border border-slate-200 hover:bg-slate-50 transition-colors cursor-pointer'
          >
            Cancel
          </button>
          <button
            ...
            disabled={loading}
            className='px-5 py-2.5 rounded-full text-xs font-semibold text-white bg-slate-900 hover:bg-black shadow-sm transition-colors cursor-pointer disabled:opacity-50'
          >
